...
Everything else — a tracked file edited, a new non-ignored file created, or a commit landing with nothing left dirty (an agent that edits *and* commits, leaving a clean working tree) — fires. `filesChangedBetween` (`git-state.ts`) diffs the old and new `HEAD` to name the files even when the working tree shows nothing dirty. When it can't (a rebase or force-push made the baseline's commit unreachable) it returns `null`, not an empty list, and the event carries `filesUnknown: true` — the change is still recorded, and the record says the file list was unavailable rather than showing an empty one as if nothing had been touched.

The baseline file itself is written through a temp file and an atomic rename (`baseline.ts`). Every `fabrica` command runs a check at startup, so two commands can overlap; an in-place rewrite would let one process read a half-written file, parse-fail, read as "never seen," and silently swallow a real change — the exact failure this net exists to prevent. `baseline.test.ts` proves this across real OS-level concurrency, the same way `src/record/` proves its appends.
