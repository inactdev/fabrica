...
tasks still moving, so it neither disappears nor looks live. `watch`
follows one task the same way: heartbeats appear live, and the worker's
own transcript arrives in a batch at the end of each attempt. Stopping
`watch` (Ctrl-C) stops only the watching; the work runs on in its own
process.
