...
The matcher is written anchored, `^(Edit|Write|NotebookEdit|MultiEdit)$`, and must stay that way. A `matcher` is a regex tested against `tool_name`, not an exact-match list, so the unanchored form `Edit|Write|...` also matches every tool whose name merely *contains* one of those words - `TodoWrite` first of all, and any MCP tool named like `mcp__x__WritePage`. Those would then be denied by the hook and written to `events.jsonl` as `edit-attempt-blocked` events, breaking harmless tools and diluting the one event this template exists to produce. The live runs above were recorded before the anchors were added; `Edit` matches identically either way, so the recorded results still hold.
