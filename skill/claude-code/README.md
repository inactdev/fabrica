...
- `settings.json` - a permissions template: wires the file-editing tools to a denying `PreToolUse` hook, and narrows Bash to an explicit list of `fabrica` subcommands that only read or start work (`do`, `status`, `log`, `watch`, `deny-and-log-edit`, `verify-hook`) plus a short allowlist of read-only ones. `fabrica verdict` and `fabrica answer` - the two commands that record the Client's own judgment - are deliberately not on that list and are additionally listed in `permissions.deny`; see "Bash: `verdict` and `answer` require the Client" below.
