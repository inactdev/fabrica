...
SPEC.md's "operator's skill": the manual and session setup for whatever AI the Client talks to. This folder currently holds only the piece issue #13 ("off-the-books nets") asks for — the per-harness session config that disables file editing for the chat agent, and its blocked-edit-attempt logging hook. The full agent-facing manual (commands, examples, taboos — SPEC.md's "The operator's skill" section) is a separate, not-yet-built piece; don't assume it's here.
...
Each harness gets its own subfolder, named after the harness, holding whatever its own permission/settings/hook mechanism needs — these are inherently harness-specific (SPEC.md: "the skill folder ships a **per-harness** session setup"), so nothing here is shared across them beyond the two Fabrica-side event types they all report into: `unattributed-change` (the detection net, `src/offbooks/`) and `edit-attempt-blocked` (this folder, where the harness supports it).
