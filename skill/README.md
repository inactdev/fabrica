...
Each harness gets its own subfolder, named after the harness, holding only what is inherently specific to it (SPEC.md: "the skill folder ships a **per-harness** session setup"): its own permission/settings/hook wiring, and a `verify.ts` exporting the real session spawn `fabrica verify-hook` drives — both halves of it, `attemptRealEdit(cwd, targetFileName)` and `harnessAvailable()`; one exporting only one of the two is skipped and named on stderr, not used. Neither half has to swallow its own failures: `attemptRealEdit` may be sync or async (it is awaited before the verdict is read) and may throw, which is reported while the verdict still comes from the file on disk and the record; a `harnessAvailable()` that throws counts as "harness unreachable", exactly as `false` does. Everything else is shared and lives in Fabrica itself, not here — the hook logic a harness's config points at is the installed `fabrica deny-and-log-edit` command, and the event it records is `edit-attempt-blocked` (`src/offbooks/blocked-edit.ts`'s `recordBlockedEditAttempt`).
